...
const WorkerProfile = () => {
  ...
  // Initialize form data from any available source
  useEffect(() => {
    console.log('WorkerProfile useEffect: initializing user data');
    
    // Try multiple sources of user data in order of preference
    const userSources = [
      { source: 'context', data: user },
      { source: 'authUtils', data: localStorageUser },
      { source: 'localStorage.user', data: (() => {
        try { return JSON.parse(localStorage.getItem('user')); } catch(e) { return null; }
      })() },
      { source: 'localStorage.INDUSUser', data: (() => {
        try { return JSON.parse(localStorage.getItem('INDUSUser')); } catch(e) { return null; }
      })() }
    ];
    
    // Find first valid user data
    let userDataSource = userSources.find(s => s.data && (s.data.id || s.data._id));
    
    if (userDataSource) {
      const userData = userDataSource.data;
      console.log(`Found user data from ${userDataSource.source}:`, userData);
      
      // Force type to worker for profile page
      if (!userData.type) {
        userData.type = 'worker';
        console.log('Forcing user type to worker for profile page');
      }
      
      console.log('Setting form data from user data source');
      ...
      const calculatedScore = calculateShaktiScore(userData);
  ...
  // Check if coming from registration - still needed for the initial edit state
  useEffect(() => {
  ...
  const calculateShaktiScore = (workerData) => {
    ...
    if (workerData.location?.address) score += 5;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Get current user data from any available source
      const currentUser = user || 
        getCurrentUser() || 
        JSON.parse(localStorage.getItem('user') || 'null') ||
        JSON.parse(localStorage.getItem('INDUSUser') || 'null') || 
        { type: 'worker' };
        
      const userId = currentUser.id || currentUser._id;
      
      // Update localStorage immediately - try multiple keys for compatibility
      const updatedUser = { ...currentUser, ...updatedFormData, type: 'worker' };
      ...
      if (updateProfile && userId) {
        try {
          await updateProfile(userId, 'worker', updatedFormData);
        } catch (apiError) {
  ...
  console.log('Render state:', { formData, isLoadingUser });
  
  // If form data has been initialized, don't show loading or error screens
  if (formData.name || formData.email || formData.phone) {
    console.log('Profile has data, rendering normal view');
  ...
  // No data and not loading - show error
  else {
  ...
  const DebugInfo = () => {
    ...
    const rawIndusUser = localStorage.getItem('INDUSUser');
    ...
    return (
      <div className="bg-yellow-100 p-4 mb-6 rounded-lg border border-yellow-300">
        ...
        <div className="text-xs space-y-1">
          <div><strong>User from Context:</strong> {user ? `${user.name} (${user.type || 'no type'})` : 'null'}</div>
          <div><strong>User from localStorage:</strong> {localStorageUser ? `${localStorageUser.name} (${localStorageUser.type || 'no type'})` : 'null'}</div>
          <div><strong>isLoadingUser:</strong> {isLoadingUser.toString()}</div>
          <div><strong>Form Data Name:</strong> {formData.name}</div>
          <div><strong>localStorage.user:</strong> {rawUser ? `exists (${rawUser.length} chars)` : 'null'}</div>
          <div><strong>localStorage.INDUSUser:</strong> {rawIndusUser ? `exists (${rawIndusUser.length} chars)` : 'null'}</div>
          <div><strong>localStorage.userType:</strong> {rawUserType || 'null'}</div>
        </div>
        <div className="mt-2 flex justify-end">
          <button 
            onClick={() => {
              ...
              localStorage.setItem('INDUSUser', JSON.stringify(testUser));
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Always show debug info for now while fixing issues */}
        <DebugInfo />
