...
const WorkerWallet = () => {
  ...
  const [walletData, setWalletData] = useState({
    balance: 0,
    withdrawableBalance: 0, // Add this
    totalEarned: 0,
    totalSpent: 0,
    transactions: []
  });
              ...
              filteredTransactions.map((transaction, index) => (
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${transaction.type === 'earning' ? 'bg-green-100' :
                  ...
                  <div className="text-right">
                    <p className={`font-bold ${transaction.type === 'earning' || transaction.type === 'bonus'
                        ? 'text-green-600'
                        : 'text-red-600'
                      }`}>
                      {transaction.type === 'earning' || transaction.type === 'bonus' ? '+' : '-'}₹{transaction.amount.toLocaleString()}
                    </p>
                    <p className={`text-xs px-2 py-1 rounded-full ${transaction.status === 'completed' ? 'bg-green-100 text-green-800' :
            ...
            >
              ...
              >
                ...
                <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-green-600">₹{(walletData.withdrawableBalance || 0).toLocaleString()}</p>
                  {walletData.balance > walletData.withdrawableBalance && (
                    <p className="text-xs text-gray-400 mt-1">Total Balance: ₹{walletData.balance.toLocaleString()}</p>
                  )}
                </div>
